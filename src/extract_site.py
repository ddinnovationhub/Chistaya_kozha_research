...
NONPROFILE_SERVICE_RE = re.compile(
    ...
    r"|фтизиатр|инфекционист|генетик[а]?\b|травматолог|ортопед|подолог"
    r"|выезд (врача|медсестры|специалиста)|на дом[у]?\b|иммуноглобулин|стельк"
    ...
    r"|беременн|внутривенн|внутримышечн|вливани|глюкометр|забор анализа"
    r"|экспресс анализ на сахар"
    r"|подкожное введение|внутрисуставн|околосуставн|ударно-волнов",
    re.IGNORECASE)
...
# ── ВТОРОЙ УРОВЕНЬ ФИЛЬТРА (архитектурный разбор заказчика 2026-08-26):
# релевантность проверяется у КАЖДОЙ ПОЗИЦИИ, независимо от клиники.
# Позиция попадает в таблицу услуг только при профильном ЯКОРЕ — дермато-
# логической лексике в названии. Сбор остаётся ОТКРЫТЫМ: незнакомая
# дерматология с якорем проходит («Ferrari-терапия невусов»); отсекается
# чужой профиль, а не отсутствие в справочнике. ───────────────────────────
