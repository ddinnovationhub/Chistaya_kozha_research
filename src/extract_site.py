...
# ── ОТМЕНА ВТОРОГО ФИЛЬТРА (заказчик, 2026-08-25): фильтрация только на
# уровне ОРГАНИЗАЦИИ. Внутри прошедшей ворота клиники не выбрасывается ни
# одна позиция ни по какому признаку; отнесение позиции к профилю — колонка
# «Профиль», заполняется по эталону заказчика, в код правила не зашиваются.
# Регэксп NONPROFILE_SERVICE_RE (правила периметра на уровне позиции) удалён. ──

# Расходники и служебные добавки к процедуре (заказчик, 2026-08-25, п.3):
# остаются в таблице с «Тип строки» = расходник, услугами не считаются
# такт 3 (2026-08-25): «лидокаин/эмла» из маркеров убраны — растворитель в
# составе инъекции («Цефтриаксон + Лидокаин», «Лонгидаза + Лидокаин» в КВД)
# помечал УСЛУГУ расходником; введение в очаг поражения кожи — профиль
CONSUMABLE_RE = re.compile(
    r"канюл|анестез|обезболив|дополнительн\w+ обработк",
    re.IGNORECASE)
...
PROCEDURE_WORD_RE = re.compile(
    ...
    r"|дерматоскоп|диагност|исследован|анализ|инъекц|введен|обработк"
    r"|эпиляц|шлифовк|плазмо|мезотерап|иссечен|криодеструкц|коагуляц|соскоб"
    # такт 3 (2026-08-25): при полном сборе короткие названия услуг любых
    # профилей — услуги, не зоны («Фотоомоложение», «Спермограмма»,
    # «Рентгенография таза», «Тампонирование боковых валиков», «ЭКГ»)
    r"|коррекц|граф|скопи|грамм|омолож|тампонир|протезир|установк|рентген"
    r"|\bузи\b|\bэкг\b",
    re.IGNORECASE)
...
# ── Якорь профиля — ТОЛЬКО счётчик для ворот ОРГАНИЗАЦИИ (заказчик,
# 2026-08-25: второй фильтр отменён, позиции якорем НЕ отсекаются).
# Якорь по названию систематически занижает долю: «Гистологическое
# исследование удаленного материала» — ядро профиля без слова «кожа».
# Это оговорено; занижение бьёт по доле в воротах, не по составу таблицы. ──
PROFILE_ANCHOR_RE = re.compile(
    # (?<!под)кож: «подкожное введение» — способ введения препарата, не
    # дерматология (такт 3: ортопедические инъекции проходили якорем)
    r"дермат|трихо|(?<!под)кож[аеиун]|волос|ногт|родин|невус|папиллом|бородав|кератом"
    r"|кондилом|моллюск|атером|липом|фибром|гемангиом|ксантелазм|ксантом"
    r"|милиум|акне|угр[еия]|прыщ|псориаз|экзем|розаце|купероз|витилиго"
    r"|алопец|облысен|себоре|перхот|демодек|гипергидроз|потливост|рубц|келоид"
    r"|растяжк|шрам|микоз|грибк|грибов|лишай|герпес|чесотк|крапивниц"
    r"|нейродермит|дерматоскоп|трихоскоп|фототрихограм|лампой? вуда"
    r"|новообразован|криодеструкц|электрокоагуляц|радиоволнов"
    r"|мезотерап|плазмотерап|prp|плазмолифт"   # мезо/плазмо: у клиента трихологические
    r"|соскоб|дерматофит|онихо|базалиом|меланом|пигмент|сальн\w+ желез",
    re.IGNORECASE)

# Строка-описание, а не услуга («Контролируемая глубина воздействия на ткани
# позволяет максимально безопасно…»)
DESCRIPTIVE_TEXT_RE = re.compile(
    r"позволя|обеспечива|благодаря|является|помога|достига|рекоменду"
    r"|наши специалист|мы использу|это современн|безопасн\w+ и |максимально",
    re.IGNORECASE)

# Заголовок страницы — НЕ имя организации («Лечение купероза на лице в
# Новосибирске: цены», «Стоимость медицинских услуг»)
HEADLINE_TITLE_RE = re.compile(
    r" в новосибирске| цены|цены,|цены:|фото до|запись на|стоимость|прайс"
    r"|^лечение |^удаление |^консультация |^контурная |^лазерн|недорого|акци[яи]",
    re.IGNORECASE)


def looks_like_headline(title: str | None) -> bool:
    return bool(title) and (bool(HEADLINE_TITLE_RE.search(title)) or len(title) > 60)


# Форма собственности (заказчик 2026-08-26, п.5): признак, не фильтр —
# КВД с платными услугами остаётся в таблице, решение по данным
_STATE_OWNERSHIP_RE = re.compile(
    r"\b(ФГБУ|ГБУЗ|ГАУЗ|МБУЗ|ГУЗ|КГБУЗ|ОГБУЗ|ФГАУ|ФКУЗ|МАУЗ|ГБУ)\b"
    r"|министерств\w* здравоохранени|департамент\w* здравоохранени")
_PRIVATE_OWNERSHIP_RE = re.compile(r"\b(ООО|АО|ЗАО|ПАО|ИП|АНО)\b")


def detect_ownership(texts: list[str], legal_name: str | None) -> str:
    """Юрлицо из реквизитов — решающее: сайт ООО, продающий путёвки в ГБУЗ-
    санатории, упоминает госформы в контенте (такт 3: putevka.com получала
    «государственная» при ООО в реквизитах)."""
    if legal_name:
        if _PRIVATE_OWNERSHIP_RE.search(legal_name):
            return "частная"
        if _STATE_OWNERSHIP_RE.search(legal_name):
            return "государственная"
    joined = "\n".join(t[:20000] for t in texts)
    if _STATE_OWNERSHIP_RE.search(joined):
        return "государственная"
    if _PRIVATE_OWNERSHIP_RE.search(joined):
        return "частная (по тексту сайта, Уточнить)"
    return "Уточнить"
...
# Приём ПРОФИЛЬНОГО врача в прайсе (заказчик, 2026-08-25, п.4): весит больше
# процедуры — клиника с приёмом дерматолога конкурент, даже если процедур мало
PROFILE_DOCTOR_VISIT_RE = re.compile(
    r"(при[её]м|консультаци|осмотр)\S*\s+(врача[-\s]*)?"
    r"(дерматолог|дерматовенеролог|онкодерматолог|трихолог|дерматохирург)",
    re.IGNORECASE)


def profile_doctor_visit_in_services(names: list[str]) -> str | None:
    """Первая строка прайса с приёмом профильного врача, или None."""
    for n in names:
        if PROFILE_DOCTOR_VISIT_RE.search(n):
            return n
    return None
...
def _esthetic_keywords(services: dict | None = None) -> list[str]:
    ...
    kws.update(["ботокс", "филлер", "фотоомоложение", "шлифовк", "омоложени",
                ...
                # класс-свёртка (заказчик 2026-08-26): аппаратные и инъекционные
                # эстетические семейства, шедшие построчно у angeliy/alleya
                "токами ультравысокой", "увч на кожу", "hifu", "смас-лифтинг",
                "введение имплантат", "коллагенотерап", "icoone",
                "атравматичн", "уход за кожей", "уходы", "дарсонвал",
                "микроток", "rf-лифтинг", "радиочастотн", "фракционн",
                "комбинированная процедура", "карбонов", "альгинатн"])
    return sorted(kws)
...
def extract_pages(pages: dict[str, str], form_index: dict,
    ...
    result["ownership_form"] = detect_ownership(
        visible_texts, result["requisites"]["legal_name"])
    return result
