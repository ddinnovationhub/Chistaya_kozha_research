...
def test_consumables_marked_not_dropped():
    ...
    инъекция препарата с лидокаином-растворителем — УСЛУГА (такт 3, кейс ngf)."""
    page = """<html><body><p>Приём врача-дерматолога — 1 500 ₽</p>
    <p>Канюля для контурной пластики — 500 ₽</p>
    <p>Аппликационная анестезия — 700 ₽</p>
    <p>Инъекционное введение лекарственных средств в очаг поражения кожи: Лонгидаза + Лидокаин 2 мл — 1 900 ₽</p>
    <p>Дерматоскопия — 1 200 ₽</p></body></html>"""
    r, db = _run_clinic(page)
    assert r["gate"] == "Включён"
    ...
    lek = [t for n, t in types.items() if "Лонгидаза" in n]
    assert lek == ["услуга"]


def test_short_service_names_are_services_not_junk():
    """Такт 3: при полном сборе короткие названия услуг любых профилей —
    услуги («Фотоомоложение», «Спермограмма», «Рентгенография таза»,
    «Чек-ап базовый»); зоны прайса остаются служебными."""
    page = """<html><body><p>Приём врача-дерматолога — 1 500 ₽</p>
    <p>Фотоомоложение — 4 000 ₽</p><p>Спермограмма — 1 000 ₽</p>
    <p>Рентгенография таза — 900 ₽</p><p>Чек-ап базовый — 9 000 ₽</p>
    <p>Подбородок — 700 ₽</p></body></html>"""
    r, db = _run_clinic(page)
    types = dict(db.execute("SELECT name_raw, row_type FROM services_found"))
    for svc in ("Фотоомоложение", "Спермограмма", "Рентгенография таза",
                "Чек-ап базовый"):
        assert types.get(svc) == "услуга", (svc, types.get(svc))
    assert types.get("Подбородок") == "служебное"


def test_claimed_derm_specialty_with_empty_price_needs_review():
    """Такт 3 (кейс naedine-n): дерм-специальность на сайте заявлена, но
    профильных позиций в малом собранном прайсе нет → «Требует проверки»
    (вероятен недобор), а не ложно-уверенное «Исключён»."""
    page = """<html><body><p>Лицензия ЛО-54-01-000001</p>
    <p>В медцентре ведут приём терапевт, гинеколог и врач-дерматолог (запись по телефону)</p>
    <p>Приём терапевта — 1 500 ₽</p><p>Приём гинеколога — 1 800 ₽</p>
    <p>Общий анализ крови — 400 ₽</p></body></html>"""
    r, db = _run_clinic(page)
    assert r["gate"] == "Требует проверки", r.get("reason")
    assert "недобор" in r["reason"]
    # прайс не-Включён клиники не тащим
    assert db.execute("SELECT COUNT(*) FROM services_found").fetchone()[0] == 0
