...
**Вордстат — способ доступа (решение заказчика, 2026-08-24):**
Только ручная выгрузка CSV заказчиком в `data/wordstat/{тег}.csv`. API Директа не используется: регистрация приложения и история открутки бюджета — недели бюрократии ради однократной задачи; Вордстат нужен один раз при сборке словаря, не на каждом прогоне. Формат колонок согласуется с заказчиком до чтения файлов. Агент читает только из `data/wordstat/`.
