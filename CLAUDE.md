...
**Обязательная регрессия на клиенте (решение заказчика, 2026-08-24).**
После КАЖДОГО изменения `classifier.yaml` или `services.yaml` прогоняется тест
`tests/test_client_regression.py`: классификатор на фактическом профиле «Чистой Кожи»
(`data/client_profile.yaml` ← ЧК_ОСИНТ_ОБРАЗЕЦ.xlsx, колонка «Оказывает ли клиент»).
Ожидание: **Тип 1, esthetic_markers_found пуст, несмежных нет.**
Тест не даёт Тип 1 → сломаны ПРАВИЛА, а не данные — менять правила.
Тест включён в шаг «Быстрые проверки» воркфлоу Actions; коммит изменений словарей
без зелёной регрессии запрещён.
...
**Политика поиска (заказчик, 2026-08-26):** теги с `use_in_search: нет`
(эстетические маркеры Типа 2, карбокситерапия) в запросы не идут — только для
распознавания на сайтах. Теги с `search_needs_profile_qualifier: да` идут в поиск
только явными фразами `search_phrases` с уточнением профиля («кожи») — процедура,
существующая вне дерматологии, без уточнения выводит не туда. Порядок L2:
группа 1 — профиль клиента, группа 2 — ядро дерм-контура, группа 3 — прочее;
внутри группы по частотности Вордстата, при её отсутствии — по алфавиту.
...
**Каскад доступа — 4 уровня строго по возрастанию стоимости (заказчик,
2026-08-26; реализация `src/fetch_cascade.py`).** Каждый следующий уровень
применяется ТОЛЬКО к тому, что не взял предыдущий:

| Уровень | Способ | Берёт |
|---|---|---|
| 1 | Jina Reader (`r.jina.ai`) | основной путь, дёшево и быстро |
| 2 | прямой HTTP: браузерный UA, заголовки, пауза | сайты, где Jina получила отказ |
| 3 | Playwright headless: networkidle, селектор, скролл до низа | SPA и ленивую подгрузку |
| 4 | Playwright, полная эмуляция: UA, viewport, языковые заголовки, случайные задержки, движение курсора | единицы антибот-сайтов |

- Каждая попытка фиксируется в `fetch_attempts` (osint.db): уровень, код,
  байты, селектор, взят ли контент — после прогона видно, какой уровень нужен.
- **suspicious_zero на странице**: ответ 200 без признаков контента (нет цен,
  нет названий услуг из справочника, объём < `cascade.content_min_bytes`) —
  НЕ взято, передаётся следующему уровню. Формальный успех ≠ успех.
- **Правовой режим на всех уровнях**: robots.txt чтится (запрет → «Сайт
  недоступен», не обход); CAPTCHA/логин не обходятся никогда; ≤1 запрос/3с.
- **Не взял ни один уровень** → строка ОСТАЁТСЯ в 01_Клиники: название/адрес/
  домен из discovery, услуги = `Сайт недоступен`, грейд C,
  `flag_site_unreachable`, в примечании последний уровень/код/дата.
  `Сайт недоступен` ≠ `Не найдено` (vocab.yaml): «не найдено» — сайт открыли,
  раздела нет; «недоступен» — сайт есть, открыть не смогли.
- Домены недоступных → `output/{город}_недоступные_{дата}.txt` на ручной обход
  заказчиком; переданное вручную вливается `python -m src.manual_import`
  с пометкой источника «ручной сбор» (только для flag_site_unreachable).
- Недоступных >15% по городу (`cascade.unreachable_share_stop`) → остановка
  и доклад: проблема системная. Статистика уровней — лист 07_Качество.

Для каждой клиники, прошедшей G1–G3:
1. Открыть через каскад (уровни 1→4)
...
**Маппинг услуг — БЕЗ внешнего API (решение заказчика 2026-08-26, п.6):**
Anthropic API не используется (расход 90–180 $ на проект неприемлем), вызовов
к внешним моделям нет ни одного. Схема:
1. Пайплайн скачивает страницы в `raw/` (как раньше).
2. Ступень 1 маппинга — кодом: нормализация + точное совпадение со справочником.
3. Несопоставленное → `output/{город}_на_разметку_{дата}.json` (по клиникам:
   дословные названия, описания, цены, URL, `row_id`).
4. Заказчик открывает Claude Code; агент размечает батчи по
   `prompts/06_markup_batch.md` → `output/{город}_разметка_{дата}.json`.
5. `python -m src.merge_markup` валидирует и сливает разметку, финализирует
   классификацию (до слияния тип клиники — предварительный, по ступени 1).
...
## ОБЯЗАТЕЛЬНЫЕ ОСТАНОВКИ НА ПРОМЕЖУТОЧНЫХ ОТЧЁТАХ (заказчик, 2026-08-26)

Агент останавливается САМ, не спрашивая разрешения и не продолжая:
- **Этап 6:** после первых 10 клиник — промежуточная выгрузка (лист 02_Услуги
  полностью, «Спорные маппинги» со средней/низкой уверенностью, «Услуги без
  тега», сводка распределения кодом/моделью/без описания/без кода 804н,
  по каждой клинике тип+флаги+грейд) и ожидание решения заказчика.
- **Этап 7:** после первых 10 обогащений реквизитами.
- **Этап 8:** Excel по первым 10 клиникам — и ждать.
Десять клиник ≈ четыреста услуг: достаточно, чтобы увидеть систематическую
ошибку, и мало, чтобы её было дорого переделать.
